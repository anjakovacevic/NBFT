...
Jedna od ključnih komponenti NBFT sistema je determinističko, ali nepredvidivo formiranje grupa. Implementirani modul \texttt{consistent\_hash.py} oslanja se na koncept konzistentnog heširanja na prstenu, uz dodatak mehanizma za balansiranje opterećenja (\textit{Bounded Load Balancing}).

Proces formiranja grupa odvija se kroz sledeće korake:

\begin{enumerate}
    \item \textbf{Mapiranje grupa}: Identifikatori grupa ($ID_G \in [0, m-1]$) se heširaju zajedno sa trenutnim brojem pogleda ($v$) kako bi se dobile njihove pozicije na virtualnom prstenu veličine $2^{32}$. Ovo osigurava da se raspored grupa menja u svakoj iteraciji (View Change).
    \[ Pos(G_i) = Hash(v || i) \mod 2^{32} \]
    
    \item \textbf{Mapiranje čvorova}: Svaki čvor ($ID_N$) se mapira na fiksnu poziciju na prstenu.
    \[ Pos(N_j) = Hash(N_j) \mod 2^{32} \]
    
    \item \textbf{Bounded Load Dodela}: Kako bi se izbegla neranomerna raspodela čvorova (koja je česta kod standardnog konzistentnog heširanja sa malim brojem ključeva), uvodi se kapacitet grupe $C_{max} = \lceil N/M \rceil + 1$.
    Algoritam iterira kroz sortirane pozicije čvorova i dodeljuje ih prvoj "narednoj" grupi na prstenu koja nije popunila svoj kapacitet. Ukoliko je najbliža grupa puna, pretraga se nastavlja ciklično dok se ne pronađe slobodno mesto.
    
    \item \textbf{Izbor predstavnika}: Unutar svake grupe, za predstavnika se bira onaj čvor čija je pozicija na prstenu "najbliža" virtualnoj tački grupe (računajući u smeru suprotnom od kazaljke na satu), čime se simulira topološka bliskost.
\end{enumerate}

Pseudokod ovog postupka prikazan je u Algoritmu \ref{alg:ch}.

\begin{algorithm}
\caption{Formiranje grupa sa Bounded Load Hashing-om}
\label{alg:ch}
\begin{algorithmic}[1]
\Function{FormGroups}{$Nodes, m, View$}
    \State $Cap \gets \lceil |Nodes|/m \rceil + 1$
    \State $Ring \gets 2^{32}$
    \State $Groups \gets \emptyset$
    \State
    \For{$i \gets 0$ \textbf{to} $m-1$}
        \State $GPos[i] \gets Hash(View || i) \mod Ring$
    \EndFor
    \State Sort $GPos$
    \State
    \For{$n \in Nodes$}
        \State $NPos \gets Hash(n.id) \mod Ring$
        \State $BestG \gets FindNextClockwise(NPos, GPos)$
        \While{$Size(Groups[BestG]) \ge Cap$}
             \State $BestG \gets NextClockwise(BestG, GPos)$
        \EndWhile
        \State Add $n$ to $Groups[BestG]$
    \EndFor
    \State \Return $Groups$
\EndFunction
\end{algorithmic}
...
Srž logike konsenzusa implementirana je kroz metodu \texttt{\_handle\_message}, koja deluje kao mašina stanja (State Machine) za svaki čvor. Stanja kroz koja čvor prolazi uključuju:
\begin{enumerate}
    \item \texttt{IDLE}: Početno stanje, čekanje na predlog.
    \item \texttt{PRE\_PREPARED}: Lokalni konsenzus unutar grupe je postignut.
    \item \texttt{PREPARED}: Globalni konsenzus (među predstavnicima) je u fazi pripreme.
    \item \texttt{COMMITTED}: Odluka je finalizovana i spremna za izvršenje.
\end{enumerate}
